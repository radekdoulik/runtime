...
- `--smoke=<name>` — run a single smoke (e.g. `--smoke=hello-cdp-pause`)
...
2. Open `http://localhost:8080/hello-cdp-pause.html` in Chrome.
...
hello-cdp-pause.html
  └─ delays startup so DevTools/CDP can enable the debugger before wasm
     instantiation, then imports hello-cdp-pause.mjs
       └─ uses host.mjs to instantiate corerun.wasm and coreclr-dbi-dac.wasm,
